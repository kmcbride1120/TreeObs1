...
GSH2018 <- SFTO[SFTO$Date < "2018-12-01",]
#boxplot.stats(GSH2018$Cor_Out, coef = 1.5, do.conf = TRUE, do.out = FALSE)
# Lower whisker: -17.9700 and upper whisker: 39.4950
#boxplot.stats(GSH2018$Cor_In, coef = 1.5, do.conf = TRUE, do.out = FALSE)
# Lower whisker: -7.674 and upper whisker: 31.122
#boxplot.stats(GSH2018$Unc_Out, coef = 1.5, do.conf = TRUE, do.out = FALSE)
# Lower whisker: -10.398 and upper whisker: 22.851
#boxplot.stats(GSH2018$Unc_In, coef = 1.5, do.conf = TRUE, do.out = FALSE)
# Lower whisker: -4.259 and upper whisker: 18.188
GSH12018 <- GSH2018[GSH2018$Cor_Out > -17.9700 & GSH2018$Cor_Out < 39.4950,]
GSH22018 <- GSH12018[GSH12018$Cor_In > -7.674 & GSH12018$Cor_Out < 31.122,]
GSH32018 <- GSH22018[GSH22018$Unc_Out > -10.398 & GSH22018$Cor_Out < 22.851,]
...
SFTOH <- rbind(GSH42018, SFH2018, GSH42019, SFH2019, GSH42020, SFH2020, GSH2021)
